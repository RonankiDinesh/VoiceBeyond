...
import { startCaptioning } from "@/lib/api";
...
export default function YoutubePage() {
    ...
    const [captions, setCaptions] = useState<{ id: string; text: string; timestamp: number }[]>([]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Send request to backend and start WebSocket connection
            const { websocket } = await startCaptioning(
                url,
                language,
                (caption) => {
                ...
                (error) => {
                    toast({
                        ...
                        description: "Lost connection to caption service. Please try again.",
                        variant: "destructive",
                    });
                    setIsConnected(false);
                }
            ...
            toast({
                title: "Error",
                description: "Failed to start captioning. Please try again.",
                variant: "destructive",
            });
        } finally {
            setIsLoading(false);
    ...
    return (
        <div className={`min-h-screen gradient-hero flex flex-col items-center justify-start px-4 py-20 font-clash relative ${isDimmed ? 'before:absolute before:inset-0 before:bg-background/80 before:backdrop-blur-sm befor:z-10' : ''}`}>
            {/* Home Button */}
            <Button
                variant="ghost"
                size="icon"
            ...
            <motion.div
                ...
                className={`w-full max-w-3xl bg-card p-8 md:p-10 rounded-2xl shadow-xl border border-border relative ${isDimmed ? 'z-20' : ''}`}
            >
                {/* Header */}
                <div className="relative w-full">
                    ...
                    <div className="flex flex-col items-center mb-10">
                        ...
                        <p className="text-muted-foreground text-lg text-center mt-2">
                            Enter the URL to preview the video and choose your caption language.
                        </p>
                    </div>
                </div>
                            ...
                            <div className="mt-6">
                                ...
                                <div className="bg-muted p-4 rounded-lg min-h-[100px] relative">
                                    <AnimatePresence mode="wait">
                                        ...
                                        >
                                            {currentCaption || 'Waiting for captions...'}
                                        </motion.div>
                                    </AnimatePresence>
                                </div>
